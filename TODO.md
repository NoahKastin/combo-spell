...
- [ ] In-game help tooltips: hover (desktop) / click-and-hold (iOS) on the Phase name shows what the player can do in it; same interaction on each Element icon in the grid shows what it does. Copy sourced from the itch.io page description, which already has player-tested phrasing for all four phases and all four MVP elements. Subsumes the "long-press any Element symbol for its definition tooltip" part of the camera-bump completion item below.
...
- [ ] Camera-bump completion (§5.3): robot-head button shell in the right notch (opens an AI/automation dialog whose content arrives with the §9 macros item). Element definition tooltips moved to the earlier in-game help tooltips item.
